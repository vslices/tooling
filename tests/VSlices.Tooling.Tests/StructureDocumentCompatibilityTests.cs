...
public sealed class StructureDocumentCompatibilityTests
{
    ...
    public async Task Structure_document_uses_the_same_progressive_authoring_mechanism_without_product_code_changes()
    {
        ...
        var created = await project.Run(
            project.Root,
            "new", "document", "architecture", "--kind", "structure");
        ...
        var initialDiscovery = await project.Run(
            project.Root,
            "discovery", "document", "architecture");
        ...
        var rootUpdate = await project.Run(
            project.Root,
            "update", "document", "architecture",
            "--question-id", "1",
        ...
        Assert.Contains(
            "vslices update document architecture --question-id 2 --answer \"<answer>\"",
            nextDiscovery.StandardOutput,
            StringComparison.Ordinal);

        var childUpdate = await project.Run(
            project.Root,
            "update", "document", "architecture",
            "--question-id", "2",
            "--answer", "La estructura del sistema y sus partes principales.");
    ...
    private static void WriteDocsStandard(string projectRoot)
    {
        ...
        File.WriteAllText(
            Path.Combine(documentsRoot, "structure-document.yml"),
            """
            kind: vslices-document-definition
            version: 0.1
